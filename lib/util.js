...
exports.parseVideo = string => {
  ...
  const thumbnail = thumbnailRaw ? thumbnailRaw : exports.between(string, 'src="', '"');
  const rawDesc = exports.between(exports.between(string, '<div class="yt-lockup-description', '</div>'), '>');
  const liveBadge = string.includes('yt-badge-live');
  return {
    type: 'video',
    ...
    views: metaInfo[1] ? Number(metaInfo[1].split(' ')[0].replace(/\.|,/g, '')) : liveBadge ? Number(metaInfo[0].split(' ')[0].replace(/\.|,/g, '')) || null : null,
    duration: exports.between(string, '<span class="video-time" aria-hidden="true">', '</span>') || null,
    uploaded_at: liveBadge ? null : metaInfo[0] || null,
    live_broadcast: liveBadge,
  };
};
