...
exports.getPage = (ref, options, cb) => {
  options = options.localAddress ?
    Object.assign(options, { localAddress: exports.getRandomIpFromRange(options.localAddress) }) : options;
...
exports.getRandomIpFromRange = range => {
  if (!IS_CIDR.v6(range)) throw new Error(`Invalid CIDR block: ${range}`);
  const address = new IP.Address6(range);
  let newAddress = address.addressMinusSuffix.split(':').filter(it => it !== '');
  const random = CRYPTO.randomBytes(newAddress.length * 2).toString('hex');
  for (let i = 0; i < random.length; i++) {
    newAddress.push(random.substring(i, i + 4));
    if (newAddress.length === 8) break;
  }
