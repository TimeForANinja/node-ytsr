 ...
 *********************************************************************/`);
  ...
  if (!options) options = { limit: 100, safeSearch: false, ipBlocks: null };
  ...
  if (options.safeSearch) requestOptions.headers = { Cookie: 'PREF=f2=8000000' };

  // Setting up IP rotation if ipBlocks is present
  if (options.ipBlocks && typeof options.ipBlocks === 'string') {
    requestOptions.localAddress = options.ipBlocks;
