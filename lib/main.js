...
const getFilters = main.getFilters = async(searchString, options) => {
  if (!searchString) throw new Error('searchString, currentRef or nextpageRef is mandatory');

  const ref = UTIL.buildRef(options.nextpageRef || options.currentRef, searchString, options);
