...
const main = module.exports = async(searchString, options) => {
  ...
  try {
    ...
    options.key = UTIL.between(body, 'INNERTUBE_API_KEY":"', '"');
    if (!options.key) options.key = UTIL.between(body, 'innertubeApiKey":"', '"');
    options.clientVersion = UTIL.between(body, 'INNERTUBE_CONTEXT_CLIENT_VERSION":"', '"');
    if (!options.clientVersion) options.clientVersion = UTIL.between(body, 'innertube_context_client_version":"', '"');
