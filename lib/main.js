...
const main = module.exports = async (searchString, options) => {
  ...
  if (Array.isArray(parsed.json.refinements)) resp.items.unshift({
  ...
  const nestedResp = await parsePage2(parsed.apiKey, parsed.clientVersion, token, opts);
...
const parsePage2 = async (apiKey, clientVersion, token, opts) => {
  DEFAULT_CONTEXT.client.clientVersion = clientVersion;

  const json = await UTIL.doPost(BASE_API_URL + apiKey, opts, { continuation: token, context: DEFAULT_CONTEXT });
  ...
  // Recursively fetch more items
  const nestedResp = await parsePage2(apiKey, clientVersion, nextToken, opts);
