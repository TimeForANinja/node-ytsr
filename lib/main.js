...
const main = module.exports = async(searchString, options) => {
  ...
  try {
    ...
    const json = UTIL_V2.parseBody(body)
    const resp2 = await UTIL_V2.mapJSON(json);
    options.limit -= resp2.items.length;
    // We're already on last page or hit the limit
    if (!resp2.nextpageToken || options.limit < 1) return resp2;
    // Recursively fetch more items
    options.nextpageToken = resp2.nextpageToken;
    const nestedResp = await UTIL_V2.parsePage2(json.apiKey, json.clientVersion, resp2.nextpageToken);
    // Merge the responses
    resp2.items.push(...nestedResp.items);
    resp2.nextpageToken = nestedResp.nextpageToken;
    return resp2;
  }
...
main.getFilters = async(searchString, options) => {
  ...
  try {
    ...
    const json = UTIL_V2.parseBody(body)
    return UTIL_V2.parseFilters(json);
  }
  const content = parsed[parsed.length - 1].body.content;
