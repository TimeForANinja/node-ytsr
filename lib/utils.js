...
exports.checkArgs = (searchString, options = {}) => {
  ...
  const inputURL = new URL(searchString);
  if (searchString.startsWith(BASE_URL) && inputURL.pathname === '/results' && inputURL.searchParams.has('sp')) {
    ...
    if (!inputURL.searchParams.get('search_query')) throw new Error('filter links have to include a "search_string" query');
    obj.query = Object.fromEntries(inputURL.searchParams);
  } else {
