...
exports.parseArtist = (wrapper) => {
  const artistDetails = [];
  const artistWrapper = wrapper
  ...
  const artistTracks = artistWrapper.sections[0].watchCardSectionSequenceRenderer.lists[0].verticalWatchCardListRenderer.items.map(track => {
    return {thumbnail: track.watchCardCompactVideoRenderer.thumbnail.thumbnails, title: track.watchCardCompactVideoRenderer.title.runs[0].text,
      ...
      videoId:track.watchCardCompactVideoRenderer.navigationEndpoint.watchEndpoint.videoId, playlistId: track.watchCardCompactVideoRenderer.navigationEndpoint.watchEndpoint.playlistId}
  });
  artistDetails.push({
