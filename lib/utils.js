...
exports.checkArgs = (searchString, options = {}) => {
  ...
  const inputQuery = URL.parse(searchString, true).query;
  if (searchString.startsWith(BASE_URL) && inputQuery.sp) {
    // Watch out for requests with a set filter
    // in such a case searchString would be an url including `sp` & `search_query` querys
    if (!inputQuery.search_query) throw new Error('filter links have to include a "search_string" query');
