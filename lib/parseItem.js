...
const parseItem = item => {
  ...
  switch (type) {
    ...
    case 'didYouMeanRenderer': // YouTube advises another query
    ...
    case 'messageRenderer':
      // some arbitrairy message - most likely "no more results"
      return null;
    ...
    case 'showingResultsForRenderer': // the results are for another query
    case 'movieRenderer':
    case 'shelfRenderer':
      return null;
    // Skip Adds for now
...
const parseDidYouMeanRenderer = obj => ({
  ...
  entrys: [{
    ...
    link: UTIL.resolve(BASE_VIDEO_URL, obj.correctedQueryEndpoint.commandMetadata.webCommandMetadata.url),
  }],
});
