...
const parseShort = obj => {
  const playerHeaderSupportedRenderer = obj.navigationEndpoint.reelWatchEndpoint.overlay
    .reelPlayerOverlayRenderer.reelPlayerHeaderSupportedRenderers;
  if (playerHeaderSupportedRenderer === undefined) {
    return parseShortLimitedData(obj);
  }
  const playerHeader = playerHeaderSupportedRenderer.reelPlayerHeaderRenderer;
...
const parseShortLimitedData = obj => {
  const navEndpoint = obj.navigationEndpoint;
  const reelPlayerOverlayRenderer = navEndpoint.reelWatchEndpoint.overlay.reelPlayerOverlayRenderer;
  return {
    type: 'short',
    title: UTIL.parseText(obj.headline, ''),
    videoID: obj.videoId,
    ...
    thumbnails: prepImg(obj.thumbnail.thumbnails),
    views: UTIL.parseText(obj.viewCountText, ''),
    published: UTIL.parseText(reelPlayerOverlayRenderer.timestamp, ''),
