...
const parseVideo = obj => {
  ...
  return {
    ...
    live: isLive,
    thumbnail: sort(obj.thumbnail.thumbnails)[0].url,
  };
};
...
const parseChannel = obj => {
  ...
  return {
    ...
    // TODO: remove when done caring about compatibility
    avatar: sort(obj.thumbnail.thumbnails)[0].url,
...
const parsePlaylist = obj => {
  ...
  return {
    ...
    thumbnail: sort(obj.thumbnails[0].thumbnails)[0].url,
    author: {
      name: owner.text,
...
const parseMovie = obj => {
  ...
  return {
    ...
    actors: !obj.bottomMetadataItems ? null : UTIL.parseText(obj.bottomMetadataItems[0]).split(': ')[1].split(', '),
    directors: !obj.bottomMetadataItems ? null : UTIL.parseText(obj.bottomMetadataItems[1]).split(': ')[1].split(', '),
    duration: UTIL.parseText(obj.lengthText),
  };
};
